...
u32 tpm2_pcr_extend_sha384(struct udevice *dev, u32 index, const uint8_t *digest)
{
        u8 command_v2[COMMAND_BUFFER_SHA384_SIZE] = {
                tpm_u16(TPM2_ST_SESSIONS),      /* TAG */
                tpm_u32(33 + TPM2_DIGEST_SHA384_LEN),  /* Length */
                tpm_u32(TPM2_CC_PCR_EXTEND),    /* Command code */
                ...
                tpm_u16(0),                     /* Size of <hmac/password> */
                                                /* <hmac/password> (if any) */
                tpm_u32(1),                     /* Count (number of hashes) */
                tpm_u16(TPM2_ALG_SHA384),       /* Algorithm of the hash */
                /* STRING(digest)                  Digest */
        };
        unsigned int offset = 33;
        int ret;
         ...
         */
        ret = pack_byte_string(command_v2, sizeof(command_v2), "s",
                               offset, digest, TPM2_DIGEST_SHA384_LEN);
        offset += TPM2_DIGEST_SHA384_LEN;
        if (ret)
                return TPM_LIB_ERROR;
...
u32 tpm2_pcr_read_sha384(struct udevice *dev, u32 idx, unsigned int idx_min_sz,
		void *data, unsigned int *updates)
{
        u8 idx_array_sz = max(idx_min_sz, DIV_ROUND_UP(idx, 8));
        u8 command_v2[COMMAND_BUFFER_SHA384_SIZE] = {
        ...
        size_t response_len = COMMAND_BUFFER_SHA384_SIZE;
        u8 response[COMMAND_BUFFER_SHA384_SIZE];
        unsigned int pcr_sel_idx = idx / 8;
        u8 pcr_sel_bit = BIT(idx % 8);
        unsigned int counter = 0;
        int ret;

        if (pack_byte_string(command_v2, COMMAND_BUFFER_SHA384_SIZE, "b",
                             17 + pcr_sel_idx, pcr_sel_bit))
                return TPM_LIB_ERROR;
        ...
        if (unpack_byte_string(response, response_len, "ds",
                               10, &counter,
                               response_len - TPM2_DIGEST_SHA384_LEN, data,
                               TPM2_DIGEST_SHA384_LEN))
                return TPM_LIB_ERROR;
